...
# TODO: At this stage, read the VCF and run vcfExtractor, ocp_cn_report, and ocp_fusion report with (virtually) no filters.  Generate
# a new data struct that is similar enough to the table generated by the convert_vcf script, but more direct.  Will require three 
# passes of the VCF file, which is a bit tedious, but should still be faster than current method?
my (%snv_indel_data, %fusion_data, %cnv_data, %ipc_data);
my $variant_data = read_vcf_new(\$vcf_file);
exit;

# Parse TSV file based on variant type and load up a hash of results.
# XXX Patch in new VCF extractor script in order to process this faster.  along those lines patch in the ocp_cn_report and ocp_fusion_report.
...
sub read_vcf_new {
    my $vcf_file = shift;
    #%snv_indel_data = proc_snv_indel_new($vcf_file);
    proc_snv_indel($vcf_file);
    #dd \%snv_indel_data;
    proc_fusion($vcf_file);
    #dd \%fusion_data;
    proc_cnv($vcf_file);
    #dd \%cnv_data;
    ...
    # XXX
    # TODO: Finish up here.
    # #%ipc_data       = proc_ipc_new($vcf_file);
...
sub proc_snv_indel {
    ...
    while (<$vcf_data>) {
        ...
        my $function   = $fields[12];
        (my $exon = $fields[11]) =~ s/Exon//;
        next if $exon eq 'intronic';
        
        if ( $vaf >= $freq_cutoff ) {
            # Anything that's a hotspot
            if ( $hotspot_id ne '.' ) {
                $snv_indel_data{$id} = gen_var_entry(\@fields, 'Hotspot Variant');
            }
            # De Novo TSG frameshift calls
            elsif ( grep {$ocp_vc eq $_} @oncomine_vc ) {
                $snv_indel_data{$id} = gen_var_entry(\@fields, 'Deleterious in TSG');
            }
            # EGFR nonframeshiftDeletion and nonframeshiftInsertion in Exon 19, 20 rule for Arms A & C
                ...
                if ( $exon == 19 && $function eq 'nonframeshiftDeletion' ) {
                    $snv_indel_data{$id} = gen_var_entry(\@fields, 'nonframeshiftDeletion in Exon 20');
                }
                elsif ($exon == 20 && $function eq 'nonframeshiftInsertion') {
                    $snv_indel_data{$id} = gen_var_entry(\@fields, 'nonframeshiftInsertion in Exon 19');
                }
            }
            # ERBB2 nonframeshiftInsertion in Exon20 rule for Arm B
            elsif ( $gene eq 'ERBB2' && $exon eq '20' && $function eq 'nonframeshiftInsertion' ) {
                $snv_indel_data{$id} = gen_var_entry(\@fields, 'nonframeshiftInsertion in Exon 20');
            }
            # KIT Exon 9 / 11 nonframeshiftInsertion and nonframeshiftDeletion rule for Arm V
                ...
                $snv_indel_data{$id} = gen_var_entry(\@fields, 'nonframeshiftIndel in Exon 9 or 11 of KIT');
            }
        }
    }
    #dd \%snv_indel_data; 
...
sub proc_cnv {
    ...
    while (<$vcf_data>) {
        ...
        $cnv_data{$fields[1]} = [@fields[0,5,8,10,9]];
    }
    return;
}
